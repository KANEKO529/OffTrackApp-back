class Api::V1::VisitRecordsController < ApplicationController
  ...
  def plot_stores
    visit_records = VisitRecord
                      .joins(:store)
                      .select('visit_records.*, stores.id AS store_id, stores.store_name, stores.latitude, stores.longitude')
                      .order(date: :desc)
    ...
    grouped_data = visit_records.group_by(&:store_id).map.with_index do |(store_id, records), index|
      {
        ...
        latitude: records.first.latitude.to_s,
        longitude: records.first.longitude.to_s,
    ...
    def with_store
      visit_records = VisitRecord.includes(:store).all.order(date: :desc)
  ...
  def visit_record_params
    params.require(:visit_record).permit(:date, :store_id, :memo)
  end
end
